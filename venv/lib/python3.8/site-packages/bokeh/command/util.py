...
It looks like you might be running the optimize_portfolio.py of a directory app directly.
...
If this is not the case, renaming optimize_portfolio.py will suppress this warning.
"""
...
def build_single_handler_application(path: str, argv: List[str] | None = None) -> Application:
    ...
    * :class:`~bokeh.application.handlers.directory.DirectoryHandler` when
      ``path`` is to a directory containing a ``optimize_portfolio.py`` script.
    ...
    Notes:
        If ``path`` ends with a file ``optimize_portfolio.py`` then a warning will be printed
    ...
    if os.path.isdir(path):
        ...
        if path.endswith(".ipynb"):
            ...
            if path.endswith("optimize_portfolio.py"):
